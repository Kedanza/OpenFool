...
assert(p ~= nil, "createPlayer should create a player object")
assert(p.name == "TestPlayer", "Player name should be 'TestPlayer'")
assert(p.index == 0, "Player index should be 0")
assert(#p.hand == 0, "New player should have empty hand")

-- Test 2: Player creation without name
print("\n-- Player Creation Without Name --")
local p2 = player.createPlayer(ruleset, nil, 1)
assert(p2.name == "", "Player without name should have empty string name")
assert(p2.index == 1, "Player index should be 1")
...
p3:addCard(c1)
assert(#p3.hand == 1, "Hand should have 1 card after adding")
p3:addCard(c2)
assert(#p3.hand == 2, "Hand should have 2 cards after adding")
assert(p3.hand[1]:equals(c1), "First card should be Ace of Spades")
assert(p3.hand[2]:equals(c2), "Second card should be King of Hearts")
...
assert(removed, "removeCard should return true when card is removed")
assert(#p4.hand == 1, "Hand should have 1 card after removal")
assert(p4.hand[1]:equals(c4), "Remaining card should be 10 of Clubs")
...
assert(not removed2, "removeCard should return false when card not found")
assert(#p4.hand == 1, "Hand size should remain unchanged")
...
assert(idx == 2, "Should find card at index 2")
local idx2 = p5:findCard(c5)
assert(idx2 == nil, "Should return nil for card not in hand")
...
p6:addCard(c2)
p6:addCard(c3)
assert(#p6.hand == 3, "Hand should have 3 cards")
p6:clearHand()
assert(#p6.hand == 0, "Hand should be empty after clearHand")

-- Test 8: Hand size
print("\n-- Hand Size Tests --")
local p7 = player.createPlayer(ruleset, "Player7", 0)
assert(p7:handSize() == 0, "New hand should have size 0")
p7:addCard(c1)
assert(p7:handSize() == 1, "Hand size should be 1")
p7:addCard(c2)
p7:addCard(c3)
assert(p7:handSize() == 3, "Hand size should be 3")
...
assert(p8.hand[1].rank == card.Rank.SIX, "First card should be SIX")
assert(p8.hand[2].rank == card.Rank.KING, "Second card should be KING")
assert(p8.hand[3].rank == card.Rank.ACE, "Third card should be ACE")
...
assert(p9.hand[1].rank == card.Rank.ACE, "First card should be ACE")
assert(p9.hand[2].rank == card.Rank.KING, "Second card should be KING")
assert(p9.hand[3].rank == card.Rank.SIX, "Third card should be SIX")
...
assert(p10.hand[1].suit == card.Suit.CLUBS, "First should be CLUBS")
assert(p10.hand[2].suit == card.Suit.HEARTS, "Second should be HEARTS")
assert(p10.hand[3].suit == card.Suit.SPADES, "Third should be SPADES")
assert(p10.hand[4].suit == card.Suit.DIAMONDS, "Fourth should be DIAMONDS (trump)")
...
assert(p11.hand[1].suit == card.Suit.DIAMONDS, "First should be DIAMONDS (trump)")
assert(p11.hand[2].suit == card.Suit.SPADES, "Second should be SPADES")
assert(p11.hand[3].suit == card.Suit.HEARTS, "Third should be HEARTS")
assert(p11.hand[4].suit == card.Suit.CLUBS, "Fourth should be CLUBS")
...
assert(p12.hand[1]:equals(originalFirst), "Order should not change with UNSORTED")
assert(p12.hand[2]:equals(originalSecond), "Order should not change with UNSORTED")
...
assert(p13:cardCanBeThrown(testCard, attackCards, defenseCards), "Any card can be thrown on first attack")
...
assert(p14:cardCanBeThrown(testCard2, attackCards2, defenseCards2), "Card with matching rank can be thrown")
...
assert(not p15:cardCanBeThrown(testCard3, attackCards3, defenseCards3), "Card with non-matching rank cannot be thrown")
...
assert(p16:cardCanBeBeaten(defenseCard, attackCards4, defenseCards4, card.Suit.SPADES), "King should beat Seven")
...
assert(not p17:cardCanBeBeaten(weakCard, attackCards4, defenseCards4, card.Suit.SPADES), "Six cannot beat Seven")
...
assert(p18:cardCanBePassed(passCard, attackCards5, defenseCards5, 6), "Card should be passable")
...
assert(not p19:cardCanBePassed(passCard, attackCards5, defenseCards5, 6), "Passing should be disabled by rules")
...
assert(not p20:cardCanBePassed(passCard, attackCards5, defenseCards6, 6), "Cannot pass after starting to defend")
...
assert(not p21:cardCanBePassed(wrongRankCard, attackCards5, defenseCards5, 6), "Cannot pass card with wrong rank")
...
assert(not p22:requireRedeal(card.Suit.SPADES), "Should not require redeal with trump")
...
assert(not p23:requireRedeal(card.Suit.SPADES), "Should not require redeal when balanced")
...
assert(p24:requireRedeal(card.Suit.SPADES), "Should require redeal when unbalanced")

-- Test 26: SortingMode enum values
print("\n-- SortingMode Enum Values --")
assert(player.SortingMode.UNSORTED == 0, "UNSORTED should be 0")
assert(player.SortingMode.SUIT_ASCENDING == 1, "SUIT_ASCENDING should be 1")
assert(player.SortingMode.SUIT_DESCENDING == 2, "SUIT_DESCENDING should be 2")
assert(player.SortingMode.RANK_ASCENDING == 3, "RANK_ASCENDING should be 3")
assert(player.SortingMode.RANK_DESCENDING == 4, "RANK_DESCENDING should be 4")
